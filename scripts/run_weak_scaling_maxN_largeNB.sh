#!/bin/bash
# Maximum N Weak Scaling with Large NB (1024)
# Uses largest possible N values with NB=1024 for potentially better performance
# Compares Native DGEMM vs GEMMul8 (Ozaki-2 INT8 emulation)
#
# Usage: ./run_weak_scaling_maxN_largeNB.sh [--native-only | --gemmul8-only]
...
# Large NB for better compute efficiency
NB=1024

# Maximum N values for native DGEMM (~216 GB per GPU)
# All values are multiples of NB=1024
N1_NATIVE=98304    # 1 GPU: 96*1024
N2_NATIVE=138240   # 2 GPU: 135*1024
N4_NATIVE=196608   # 4 GPU: 192*1024
N8_NATIVE=277504   # 8 GPU: 271*1024

# GEMMul8 N values (~93 GB per GPU + workspace)
N1_GEMMUL8=64512   # 1 GPU: 63*1024
N2_GEMMUL8=91136   # 2 GPU: 89*1024
N4_GEMMUL8=129024  # 4 GPU: 126*1024
N8_GEMMUL8=182272  # 8 GPU: 178*1024

# GEMMul8 moduli count
GEMMUL8_MODULI=12

# Results file
RESULTS_FILE="weak_scaling_maxN_NB${NB}_$(date +%Y%m%d_%H%M%S).txt"
...
print_header "Maximum N Weak Scaling with Large NB=${NB}"
echo ""
...
echo "Block size: NB=$NB (larger block for better compute efficiency)"
echo ""
...
if [ "$RUN_NATIVE" == "true" ]; then
    print_header "Native DGEMM (Maximum N, NB=$NB, ~216 GB per GPU)"
    echo ""
...
if [ "$RUN_GEMMUL8" == "true" ]; then
    print_header "GEMMul8 (${GEMMUL8_MODULI} moduli, NB=$NB, ~93 GB per GPU + workspace)"
    echo ""
