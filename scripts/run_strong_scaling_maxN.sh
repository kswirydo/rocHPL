#!/bin/bash
# Maximum N Strong Scaling Benchmark Script for rocHPL
# Fixed large N, varying number of GPUs
# Compares Native DGEMM vs GEMMul8 (Ozaki-2 INT8 emulation)
#
# Usage: ./run_strong_scaling_maxN.sh [--native-only | --gemmul8-only]
...
# For strong scaling, use fixed N that fits in minimum GPU count
# Native: Use N that fits in 1 GPU (~216 GB)
N_NATIVE=98304

# GEMMul8: Use N that fits in 1 GPU with workspace (~94 GB + workspace)
N_GEMMUL8=64896
...
# Results file
RESULTS_FILE="strong_scaling_maxN_$(date +%Y%m%d_%H%M%S).txt"
...
print_header "Maximum N Strong Scaling Benchmark"
echo ""
...
if [ "$RUN_NATIVE" == "true" ]; then
    print_header "Native DGEMM (N=$N_NATIVE)"
    echo ""
...
if [ "$RUN_GEMMUL8" == "true" ]; then
    print_header "GEMMul8 (N=$N_GEMMUL8, ${GEMMUL8_MODULI} moduli)"
    echo ""
